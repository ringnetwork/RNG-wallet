...
angular.module('ringnetworkApp.controllers').controller('correspondentDeviceController',
                    ...
                    function composeAndSend(shared_address, arrDefinition, assocSignersByPath, my_address) {
                        ...
                        fc.sendMultiPayment(opts, function (err) {
                            ...
                            if (contract.peer_pays_to === 'contract') {
                                ...
                                paymentRequestCode = 'RNG:' + my_address + '?amount=' + peer_amount + '&asset=' + encodeURIComponent(contract.peerAsset);
		...
		function showRequestPaymentModal(myPaymentAddress) {
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				...
				$scope.submitForm = function (form) {
					...
					appendText('[' + amount + ' ' + units + '](RNG:' + myPaymentAddress + '?' + params + ')');
					$modalInstance.dismiss('cancel');
				};
